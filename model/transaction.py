...
class CashFlow(models.Model):
    ...
    master_id = fields.Many2one(
        'hrsystem.transaction', required=True, copy=False)
...
class transaction_master(models.Model):
    ...
    def get_month_selection(self):
        months = [(str(i), datetime.date(1900, i, 1).strftime('%B'))
                  for i in range(1, 13)]
        return months

    month = fields.Selection(
        selection='get_month_selection', string='الشهر', required=True)
    ...
    def convirm(self):
        ...
        for recorde in empolyees:
            ...
            product_loan = self.env['product.product'].search(
                [('products_select', '=', 'prod_on_account')])
            product_loan_object = {
            ...
            print("========loan==============",get_employee_detail.loan)
            invoice_details.append((0, 0, product_loan_object))

            product_rewards = self.env['product.product'].search(
                [('products_select', '=', 'prod_reward')])
            ...
            product_off_days = self.env['product.product'].search(
                [('products_select', '=', 'prod_off_days')])
        ...
        message = [ {'type': 'ir.actions.client',
            'tag': 'reload', },{
    ...
    def display_employees(self):
        empolyees = self.env['hr.employee'].search(
            [('emplo_checkbox', '=', 'True')])
        employee_isExist = self.env['hrsystem.transactiondetails'].search(
